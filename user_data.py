...
def predict_user_gender(token):
    ...
    X_train, X_val, y_train, y_val = train_test_split(
        X, y, test_size=0.2, stratify=y, random_state=39
    )
    ...
    df_bow_test = pd.DataFrame(
        bow.todense(), columns=text_transformer.get_feature_names()
    )
    # Predict probability
...
def user_tweets(user_id):
    ...
    for column in range(0, tweets_df["Text"].size):
        tweets_df.loc[column, "Text"] = translator.translate(
            tweets_df["Text"].iloc[column]
        ).text
